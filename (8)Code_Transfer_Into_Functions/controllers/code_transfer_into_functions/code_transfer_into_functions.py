...
def grid_search(robot):
    ...
    # Main loop:
    while robot.step(grid_search.time_step) != -1:# or grid_search.exit_flag != 1:
        ...
        # If connot find any target position find another position
        if(target_pos_search.found == 0):
            # Return max State History Sequence
            state_history_maxSeq = 0
            brk = 0
            while(state_history_maxSeq < update_map.state_history_length and brk == 0):
                if(update_map.state_history_seq[state_history_maxSeq] > 16):
                    brk = 1
                else:
            ...
            while(state_history_maxSeq >= 0):
                check_x = update_map.state_history_x[state_history_maxSeq]
                check_y = update_map.state_history_y[state_history_maxSeq]
                
                if(grid_search.grid_map[check_x + 1][check_y] == 0):
                    grid_search.target_temp_pos_seq = state_history_maxSeq
                    target_temp_pos_direction = "north"
                    route_found = 1
                
                if(grid_search.grid_map[check_x][check_y + 1] == 0 and route_found == 0):
                    grid_search.target_temp_pos_seq = state_history_maxSeq
                    target_temp_pos_direction = "east"
                    route_found = 1
                            
                if(grid_search.grid_map[check_x - 1][check_y] == 0 and route_found == 0):
                    grid_search.target_temp_pos_seq = state_history_maxSeq
...
# =============================================================================
    ...
    if(grid_search.grid_map[grid_search.cur_pos_x][grid_search.cur_pos_y] == 0):
        ...
        update_map.state_history_x[update_map.seq_counter] = grid_search.cur_pos_x
        update_map.state_history_y[update_map.seq_counter] = grid_search.cur_pos_y
        update_map.state_history_seq[update_map.seq_counter] = update_map.seq_counter
        update_map.seq_counter += 1
        # Print updated map
...
# =============================================================================
    ...
    if(target_pos_search.found == 1 and cal_next_step_range.state == 0):
...
if __name__ == "__main__":
    ...
    update_map.state_history_length = 16
    update_map.state_history_x = [0 for i in range(update_map.state_history_length)]
    update_map.state_history_y = [0 for i in range(update_map.state_history_length)]
    update_map.state_history_seq = [20 for i in range(update_map.state_history_length)]
    update_map.seq_counter = 0
