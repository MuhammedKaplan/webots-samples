...
def grid_search(robot, new_x, new_y):
    ...
    while (robot.step(grid_search.time_step) != -1 and grid_search.exit_flag != 1):
        ...
        while grid_search.receiver.getQueueLength()>0:            
            ...
            if message_byte[0] == -1 and grid_search.extend_check == 0:
                
                comming_message = grid_search.receiver.getData()
                dataList = struct.unpack("h",comming_message)
                
                if dataList[0] == 0: search_area.target_direcition = "north"
                if dataList[0] == 1: search_area.target_direcition = "east"
                if dataList[0] == 2: search_area.target_direcition = "south"
                if dataList[0] == 3: search_area.target_direcition = "west"
...
# =============================================================================
    ...
    if(target_pos_search.message_send == 1):
        ...
        byte = 3
        message = struct.pack("hhh",grid_search.grid_map[grid_search.cur_pos_x][grid_search.cur_pos_y]
                              ,grid_search.cur_pos_x, grid_search.cur_pos_y)
