...
# =============================================================================
    ...
    if(grid_search.grid_map[grid_search.cur_pos_x][grid_search.cur_pos_y] == 0
       or grid_search.grid_map[grid_search.cur_pos_x][grid_search.cur_pos_y] == 1):
        ...
        grid_search.grid_map[grid_search.cur_pos_x][grid_search.cur_pos_y] = 4
    ...
    while grid_search.receiver.getQueueLength()>0:            
        ...
        if message_byte[0] == -1:
            ...
            for i in range(1, len(dataList), 3):
                ...
                if(grid_search.grid_map[update_pos_x][update_pos_y] != 4):
                    if((grid_search.grid_map[update_pos_x][update_pos_y] == 5 and dataList[0] == 5)
...
# =============================================================================
    ...
    # print(target_pos_search.sng_state,target_pos_search.found,target_pos_search.target_found)
    if(target_pos_search.sng_state == 0 and target_pos_search.found == 0 
       and target_pos_search.target_found == 0 and grid_search.final_state == 0):
        ...
        if (target_pos_search.path != None):
            ...
            message = struct.pack("hhhh",4,1,target_pos_search.sng_last_x,target_pos_search.sng_last_y)
            byte = 4
            byte_message = struct.pack("h",byte)
            grid_search.emitter.send(byte_message)
            grid_search.emitter.send(message)
    ...
    if(target_pos_search.message_send == 1):
        ...
        if(grid_search.grid_map[target_pos_search.x][target_pos_search.y] == 0
           or grid_search.grid_map[target_pos_search.x][target_pos_search.y] == 1):
            grid_search.grid_map[target_pos_search.x][target_pos_search.y] = 4
            print("my new target:")
            draw_map(grid_search.grid_map)
        ...
        byte = 4
        message = struct.pack("hhhh",4,grid_search.grid_map[grid_search.cur_pos_x][grid_search.cur_pos_y]
                              ,grid_search.cur_pos_x, grid_search.cur_pos_y)
        for j in range(len(grid_search.grid_map[0])):
            for i in range(len(grid_search.grid_map)):
                if(grid_search.grid_map[i][j] == 1 or grid_search.grid_map[i][j] == 4
        ...
        if(search_area.found == 0 and search_area.r5_final == 1
           and search_area.r6_final == 1 and search_area.r7_final == 1):            
            ...
            target_pos_search.path = search_near_grid(grid_search.grid_map, 
                                                      (grid_search.cur_pos_x, grid_search.cur_pos_y))            
            print(target_pos_search.path)
            while target_pos_search.path == None:
                receive_message()
                target_pos_search.path = search_near_grid(grid_search.grid_map, 
                                                      (grid_search.cur_pos_x, grid_search.cur_pos_y))
            ...
            if (target_pos_search.path != None):
                ...
                message = struct.pack("hhhh",4,1,x,y)
                byte = 4
                byte_message = struct.pack("h",byte)
                grid_search.emitter.send(byte_message)
                grid_search.emitter.send(message)
                print("sng check(final)")
    ...
    while queue:
        ...
        for x2, y2 in ((x+1,y), (x-1,y), (x,y+1), (x,y-1)):
            if (0 <= x2 < rows and 0 <= y2 < cols and 
                (grid_map[x2][y2] == 4 or grid_map[x2][y2] == 1 or grid_map[x2][y2] == 0)
...
# =============================================================================
    ...
    # Else go forward
        ...
        if(grid_search.grid_map[target_pos_search.x][target_pos_search.y] == 5
           ...
           or grid_search.ds_front_value < 250 or grid_search.ds_front_l < 260
           or grid_search.ds_front_r < 260):
            ...
            if (rotate_check.counter == 0 and target_pos_search.path == []):
                target_pos_search.sng_state = 0
                target_pos_search.target_found = 0
                cal_next_step_range.state = 0
        ...
        # elif(grid_search.ds_front_value < 280 and 
        #    ((-0.01<grid_search.north<0.01 and grid_search.grid_map[grid_search.cur_pos_x + 1][grid_search.cur_pos_y] != 3)
        #     or (1.56<grid_search.north<1.58 and grid_search.grid_map[grid_search.cur_pos_x][grid_search.cur_pos_y + 1] != 3
        #     or ((-3.15<grid_search.north<-3.13 or 3.13<grid_search.north<3.15) and grid_search.grid_map[grid_search.cur_pos_x][grid_search.cur_pos_y + 1] != 3)
        #     or (-1.58<grid_search.north<-1.56 and grid_search.grid_map[grid_search.cur_pos_x][grid_search.cur_pos_y + 1] != 3)))):
        #      grid_search.left_speed = 0
        #      grid_search.right_speed = 0
        #      print("waiting")
        #      if rotate_check.counter == -1:   rotate_check.counter = 400
        #      if rotate_check.counter > 0:    rotate_check.counter -= 1
        #      if (rotate_check.counter == 0 and target_pos_search.path
        #          and grid_search.final_state == 0):
        #          target_pos_search.sng_state = 0
        #          target_pos_search.target_found = 0
        #          cal_next_step_range.state = 0
        #          target_pos_search.direction = "empty"
        #          x, y = target_pos_search.path[-1]
        #          grid_search.grid_map[x][y] = 0
...
def search_area(robot):
    ...
    while search_area.mission_complate != 1:
        ...
        # print(search_area.cur_x, search_area.cur_y)
        # print(search_area.target_x, search_area.target_y)
    ...
    print("amanda aman nerelerdeydik nerelere geldik")

if __name__ == "__main__":
    # Call robot and define robot variable
    my_robot = Robot()
    
    grid_search.const_counter = 70
