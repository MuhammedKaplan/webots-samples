...
def receive_message():
    ...
    while grid_search.receiver.getQueueLength()>0:            
        ...
        if message_byte[0] == -1:
            ...
            comming_message = grid_search.receiver.getData()
            dataList = struct.unpack("hhhf",comming_message)
            
            ok = 1
            
            if(dataList[1] == target_pos_search.x and dataList[2] == target_pos_search.y):
                if(target_pos_search.target_time < dataList[3]):
                    ok = 0
            
            if ok == 1:   grid_search.grid_map[target_pos_search.x][target_pos_search.y] = dataList[0]
            
            byte = -4
    
            message = struct.pack("hh",dataList[0],ok)
...
# =============================================================================
    ...
    if(target_pos_search.target_found == 1 and target_pos_search.target_state == 1):
        byte = -3
        
        message = struct.pack("hhhf",4,target_pos_search.x,target_pos_search.y,target_pos_search.target_time)
    ...
    # Else go forward
        ...
        if(grid_search.grid_map[target_pos_search.x][target_pos_search.y] == 5
           ...
           or grid_search.grid_map[target_pos_search.x][target_pos_search.y] == 7):
            # if((grid_search.ds_front_value < 250 or grid_search.ds_front_l < 260 or grid_search.ds_front_r < 260)):
                grid_search.left_speed = 0
                grid_search.right_speed = 0
                # print("waiting")
