...
def cal_next_grid():
    if(cal_next_grid.found == 0):
        if(0 <= search_area.cur_x < search_area.rows - 1 and 0 <= search_area.cur_y < search_area.cols - 1):
            print("calculate next grid")
            if(search_area.map[search_area.cur_x + 1][search_area.cur_y] == 0):
            ...
            if(search_area.map[search_area.cur_x][search_area.cur_y + 1]):
            ...
            if(search_area.map[search_area.cur_x - 1][search_area.cur_y]):
                search_area.target_x = search_area.cur_x - 1
                search_area.target_y = search_area.cur_y
